...
const insertArticles = (articles) => async () => {
  ...
  const result = await db.query(
    pgFormat(
      'INSERT INTO articles (author, topic, title, body, image_url, upvote_count, created_at) VALUES %L RETURNING *;',
      formattedarticles.map((art) => [
        ...
        art.body,
        art.image_url,
        art.upvote_count || 0, // to do with the way test seed data and dev seed data are different
        art.created_at,
      ])
    )
  )
